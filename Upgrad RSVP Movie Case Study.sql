...
/* 1059 is the total no.of movies where either USA or India was involved. */
...
/* Finding genres of top avg rating movies */
select movies_avg_rating.title,g.genre from (
SELECT title, avg_rating,
		DENSE_RANK() OVER(ORDER BY avg_rating DESC) AS movie_rank
FROM movie AS m
INNER JOIN ratings AS r
ON r.movie_id = m.id
LIMIT 10 ) movies_avg_rating
join movie AS m
on movies_avg_rating.title = m.title
join genre g
on g.movie_id=m.id
order by movies_avg_rating.avg_rating desc;
...
SELECT d.name_id, name, d.movie_id,
	   m.date_published, 
       LEAD(date_published, 1) OVER(PARTITION BY d.name_id ORDER BY date_published ASC, d.movie_id) AS next_movie_date
